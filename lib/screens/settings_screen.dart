            ...
            GlassPanel(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      const Icon(Symbols.security, color: Colors.white54, size: 24),
                      const SizedBox(width: 12),
                      Text('Уровень защиты (Security Guard)', style: Theme.of(context).textTheme.titleSmall),
                    ],
                  ),
                  ...
                  GridView.count(
                    ...
                    children: [
                      _buildToggleItem(
                        context,
                        'System Kill Switch',
                        'Блокирует весь трафик при разрыве соединения.',
                        config.killSwitch,
                        (val) => notifier.updateSettings(killSwitch: val),
                      ),
                      _buildToggleItem(
                        context,
                        'Auto-Connect',
                        ...
                        config.autoConnect,
                        (val) => notifier.updateSettings(autoConnect: val),
                      ),
                      _buildToggleItem(
                        context,
                        'LAN Visibility',
                        'Доступ к локальным устройствам (принтеры, NAS).',
                        config.lanVisibility,
                        (val) => notifier.updateSettings(lanVisibility: val),
                      ),
                      _buildToggleItem(
                        context,
                        'Custom DNS',
                        'Использование зашифрованных DNS-серверов.',
                        config.customDns,
                        (val) => notifier.updateSettings(customDns: val),
                      ),
                    ],
                  ),
                ],
